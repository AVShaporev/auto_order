...
class Thread1(QtCore.QThread):
    ...
    def run(self):
        self.exec_()                                    # запуск цикла обработки сигналов
...
class Thread2(QtCore.QThread):
    ...
    def run(self):
        self.exec_()                                    # запуск цикла обработки сигнала

    def on_change(self, i):
        i += 10
        self.s2.emit(f'Число - {i}')
...
if __name__ == '__main__':
    ...
    window = MyWindow()
    window.setWindowTitle('Пример обмена сигналами мекжду потоками')
